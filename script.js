...
// Komentar lucu saat absen
const komentarLucu = [
  "Wah kamu rajin banget, jangan-jangan titisan malaikat 👼",
  "Absennya cepet amat, kamu ngintip jam ya? ⏰",
  "Kamu hadir terus, istirahat juga penting loh 😅",
  "Semangat terus, calon lulusan cumlaude nih! 🎓",
  "Kalau kamu lebih rajin dari dosen, itu tandanya kamu OP 🔥",
  "Rajin amat, jangan-jangan ini tugas rahasia dari NASA 🛰️",
  "Kamu hadir terus... ada niat buat tidur di kampus? 💤"
];

// Fungsi untuk komentar lucu mahasiswa saat absen
function tampilkanKomentarLucu() {
  const komentar = komentarLucu[Math.floor(Math.random() * komentarLucu.length)];
  alert("✅ Absensi Berhasil!\n\n" + komentar);
}

// Fungsi hitung persentase kehadiran mahasiswa
function hitungPersentase(mahasiswa) {
  let totalPertemuan = 0;
  let totalHadir = 0;

  semuaMataKuliah.forEach(matkul => {
    if (dataAbsensi[mahasiswa] && dataAbsensi[mahasiswa][matkul]) {
      const data = dataAbsensi[mahasiswa][matkul];
      totalHadir += data.hadir.length;
      totalPertemuan += data.hadir.length + data.tidakHadir.length;
    }
  });

  const persen = totalPertemuan > 0 ? Math.round((totalHadir / totalPertemuan) * 100) : 0;
  return persen;
}

// Fungsi komentar berdasarkan persentase
function komentarPersentase(persen) {
  if (persen === 100) return "Kamu sangat rajin sekali! Jangan-jangan kamu robot 🤖";
  if (persen >= 85) return "Rajin banget! Kamu layak dapat beasiswa 💰";
  if (persen >= 70) return "Lumayan rajin, tapi jangan kendor 💪";
  if (persen >= 50) return "Rajin bolos ya kamu 😒";
  return "Waduh, kamu kuliah online dari mimpi ya? 😴";
}

// ===== UNTUK ADMIN: STATISTIK UMUM =====
function tampilkanStatistikAdmin() {
  let container = document.getElementById('tables');
  const stats = document.createElement('div');
  stats.innerHTML = "<h3>📊 Statistik Kehadiran Mahasiswa</h3>";

  let dataStat = [];

  Object.keys(dataAbsensi).forEach(nama => {
    const persen = hitungPersentase(nama);
    dataStat.push({ nama, persen });
  });

  dataStat.sort((a, b) => b.persen - a.persen);
  const top = dataStat[0];
  const low = dataStat[dataStat.length - 1];

  stats.innerHTML += `
    <p>Mahasiswa Terajin: <strong>${top.nama}</strong> (${top.persen}%)</p>
    <p>Mahasiswa Termalas: <strong>${low.nama}</strong> (${low.persen}%)</p>
  `;
